<template>
  <b-container fluid style="padding: 80px 40px 0 40px">
    ...
    <div class="mt-3">Selected file: {{ itemImage ? itemImage.name : '' }}</div>
    <br/>
    <b-form-file
        v-model="itemImage"
    ...
    <div style="padding-top: 10px">
        ...
        <ul>
            <v-btn v-on:click='removeTag(tag.name)' style='margin-right:20px' v-for='tag in tags' v-bind:key='tag.name'>{{tag.name}}</v-btn>
        </ul>
    </div>
...
<script>
    export default {
        ...
        methods: {
            successUpload(){
            this.$vs.notify({color:'success',title:'Upload Success',text:'Upload Suceeded'})
            },
            addTag() {
